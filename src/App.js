...
class App extends Component {
  ...
  performSearch(searchTerm = this.state.searchTerm) {
    const urlString =
      "https://api.themoviedb.org/3/search/movie?api_key=ad8a2a3e22a31453b48785c80f462afd&query=" +
      searchTerm;
    $.ajax({
      url: urlString,
      ...
      success: (searchResults) => {
        ...
        results.forEach((movie) => {
          ...
          this.setState({
            ...
            homepage:
              movie.homepage === "" || movie.homepage === 0
                ? "-"
                : movie.homepage,
            poster_src:
              movie.poster_path == null
            ...
            revenue:
              movie.revenue === "undefined" || movie.revenue === 0
                ? "-"
                : "$" + this.numberWithCommas(movie.revenue),
            backdrop:
  ...
  render() {
    const jer = {
      background:
        "linear-gradient(rgba(0,0,0,0.9),rgba(0,0,0,0.3),rgba(0,0,0,0.3),rgba(0,0,0,0.9)),url(" +
        this.state.backdrop +
        ")",
    };
